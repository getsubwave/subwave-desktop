...
fn onCommand(name: []const u8) ?Msg {
    ...
    if (std.mem.eql(u8, name, "sleep-cycle")) return .{ .sleep_pick = 30 };
    if (std.mem.eql(u8, name, "cancel-sleep")) return .cancel_sleep;
...
fn statusItem(m: *const Model, scratch: *model.App.StatusItemScratch) model.App.StatusItemState {
    ...
    n += 1;
    scratch.items[n] = .{ .id = 5, .label = m.play_label(), .command = "toggle-play" };
    n += 1;
    scratch.items[n] = .{ .id = 6, .label = m.mute_label(), .command = "toggle-mute" };
    n += 1;
    scratch.items[n] = .{ .id = 7, .separator = true };
    n += 1;
    // Item labels must point at stable memory (the scratch holds slices, and
    // its one buffer is the title's) — so the armed state gets a static label.
    if (m.sleep_armed()) {
        scratch.items[n] = .{ .id = 8, .label = "Cancel sleep timer", .command = "cancel-sleep" };
    } else {
        scratch.items[n] = .{ .id = 8, .label = "Sleep timer: 30 min", .command = "sleep-cycle" };
    }
    n += 1;
...
test "status-item menu derives from the model and maps commands back" {
    ...
    try testing.expectEqualStrings("Tune out", state.items[3].label);
    try testing.expect(onCommand(state.items[3].command).? == .tune_toggle);
    try testing.expectEqualStrings("Pause", state.items[4].label);
    try testing.expect(onCommand(state.items[4].command).? == .toggle_play);
